...
private:
     ...
     */
    void modifyPixel(Mat &input, int row, int col, Color color);

    /**
     * compute color distance dist between a and b, and return dist<colorThreshold
     * @param a a BGR color
     * @param b a BGR color
     * @param colorThreshold
     * @return return true if dist<colorThreshold
     */
    bool colorDistance(const Vec3b &a, const Vec3b &b, int colorThreshold);
};
